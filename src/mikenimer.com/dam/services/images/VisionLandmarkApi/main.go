...
func RequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if len(apiResults) > 0 {
			...
			if err2 == nil {
				gcp.SaveMetadataFile(_bucket, _name, "visionapi.landmarks.json", resultsJson)
			} else {
				log.Println("Error parsing VisionAPI Landmarks | " + err2.Error())
			}
		}
...
func callVisionApi(bucket string, name string) ([]*vision3.EntityAnnotation, error) {
	...
	results, err := client.DetectLandmarks(ctx, image, nil, 10)
	if err != nil {
