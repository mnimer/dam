...
type ExplicitApiResult struct{
	Bucket	string			`json:"bucket"`
	Name 	string			`json:"name"`
	DateTime time.Time		`json:"datetime"`
	Metadata *vision3.SafeSearchAnnotation	`json:"metadata"`
}
...
func RequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	if( err != nil ){
		...
		if (apiResults != nil) {
			...
			if (err2 == nil) {
				gcp.SaveMetadataFile(_bucket, _name, "visionapi.explicit.json", resultsJson)
			} else {
				log.Println("Error parsing VisionAPI Faces | " + err2.Error())
			}
		}
...
func callVisionApi(bucket string, name string) (*vision3.SafeSearchAnnotation, error) {
	...
	results, err := client.DetectSafeSearch(ctx, image, nil)
	if err != nil {
