...
type PropertiesApiResult struct{
	Bucket	string			`json:"bucket"`
	Name 	string			`json:"name"`
	DateTime time.Time		`json:"datetime"`
	Metadata *vision3.ImageProperties	`json:"metadata"`
}
...
func RequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	if( err != nil ){
		...
		if (len(apiResults.DominantColors.Colors) > 0) {
			resultsJson, err2 := json.Marshal(apiResults)
			if (err2 == nil) {
				gcp.SaveMetadataFile(_bucket, _name, "visionapi.properties.json", resultsJson)
			} else {
				log.Println("Error parsing VisionAPI Properties | " + err2.Error())
			}
		}

		//return if called via http
		w.WriteHeader(http.StatusOK)
		m :=  PropertiesApiResult{}
		m.Name = _name
		m.Bucket = _bucket
		m.DateTime = time.Now()
...
func callVisionApi(bucket string, name string) (*vision3.ImageProperties, error) {
	...
	results, err := client.DetectImageProperties(ctx, image, nil)
	if err != nil {
